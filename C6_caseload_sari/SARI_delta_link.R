...
SARI_link_reg <- function(seed, SARI_z, Nowcast_list,population, S, R, D, D_max, n_knots, n_iter, n_burn, n_thin, n_chains){
  # Load libraries.
  library(nimble)
  library(tidyverse)
  library(mgcv)
  ...
  SARI_inits <- list(
    ...
    delta=rnorm(S,0,0.1),#matrix(rnorm(D*S,0,0.1),ncol=D),
  ...
  for(s in 1:S){
    ...
    SARI_mcmc_config$addSampler(target=c( paste('delta[',s,']', sep=''),paste('kappa_eta[1:',(SARI_constants$K_t2),', ',s,']', sep=''),paste('sigma_eta[',s,']',sep=''),
...
# Run Cluster.
time_link_delta <- system.time({
  SARI_link_output_delta <-  parLapply(cl = this_cluster_reg_age, X = 1:length(Nowcast_list), 
                                      fun = SARI_link_reg,
                                      SARI_z=as.array(SARI_link_z[1:S,,,]),
...
save(SARI_link_output_delta,time_link_delta, file='survivor_link27.RData')
...
for(k in 1:length(sari_parameter_group)){
  ...
  for(j in 1:length(Nowcast_list)){
    sari_parameter_names <- colnames((as.matrix(SARI_link_output_delta[[j]])))
    sari_index <- which(startsWith(sari_parameter_names,sari_parameter_group[k]))
    sari_psrf[[k]][[j]]<- sapply(sari_index,function(v)gelman.diag(SARI_link_output_delta[[j]][,v],autoburnin = FALSE,transform = TRUE,multivariate = FALSE)$psrf[,1])
...
plot(SARI_link_output_delta[[1]][,"lambda[1, 1]"],main="lambda[1, 1]")
plot(SARI_link_output_delta[[2]][,"xi[10, 1]"],main="xi[10, 1]")
plot(SARI_link_output_delta[[2]][,"xi[1, 1]"],main="xi[1, 1]")
plot(SARI_link_output_delta[[1]][,"delta[5]"],main="delta[5]")
plot(SARI_link_output_delta[[2]][,"delta[1]"],main="delta[1]")

#### Plots #####
library(geobr)
states<-read_state(year=2013)
n_sim<-dim(SARI_link_output_delta[[1]]$chain1)[1]*2
SARI_Survivor_link_x<-list()
SARI_combined_Survivor_link_x<-list()
surv_link_mu_x<-covid_surv_link_quant_x<-sari_surv_link_quant_x<-list()
...
for(j in length(Nowcast_list):1){
  # Survivor age model
  SARI_Survivor_link_x[[j]]<-as.mcmc.list(SARI_link_output_delta[[j]])
  SARI_combined_Survivor_link_x[[j]] <- as_tibble(do.call('rbind',SARI_Survivor_link_x[[j]]))
}
...
delta_raw<-y_raw<-list()
# Explore output
n_sim <- dim(SARI_combined_Survivor_link_x[[1]])[1]
rbetabin=nimbleFunction(run=function(n=integer(0),mu=double(0),phi=double(0),size=double(0)){
...
# Plot Model Variables 
  lambda[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('lambda'))%>%as.matrix()%>%array(dim=c(n_sim,Nowcast_list[j],S))%>%
  ...
  SARI_logspline_quantiles[[j]]<-select(SARI_combined_Survivor_link_x[[j]],starts_with('y'))%>%as.matrix()%>%array(dim=c(n_sim,S,Nowcast_list[j]))%>%
  ...
  # Plot Zeta Variables 
  zeta[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('zeta1'))%>%as.matrix()%>%array(dim=c(n_sim,Nowcast_list[j],S))%>%
    apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','t','s'),value.name='y')%>%#mutate(t=t+Nowcast_list[j]-W)%>%
    spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_units[s])
  ...
  delta[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('delta'))%>%as.matrix()%>%array(dim=c(n_sim,S))%>%
    apply(c(2),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','s'),value.name='y')%>%
    spread(quantile,y)%>%mutate(Nowcast_date=as.numeric(paste(Nowcast_list[j])),s=federal_units[s])#%>%filter(s%in%fu_abbr)
  ...
  xi[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('xi'))%>%as.matrix()%>%array(dim=c(n_sim,52,S))%>%
  ...
  #omega[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('omega'))%>%as.matrix()%>%array(dim=c(n_sim,W,S))%>%
  #  apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','t','s'),value.name='y')%>%mutate(t=t+Nowcast_list[j]-W)%>%
  # spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_units[s])
  ...
  psi[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('psi'))%>%as.matrix()%>%array(dim=c(n_sim,D,S))%>%
  ...
  psi_plot<-psi_plot+
   ...
   eta[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('eta'))%>%as.matrix()%>%array(dim=c(n_sim,Nowcast_list[j],S))%>%
    apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','t','s'),value.name='y')%>%#mutate(t=t+Nowcast_list[j]-W)%>%
    spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_units[s])
  ...
  eta_raw[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('eta'))%>%as.matrix()%>%array(dim=c(n_sim,Nowcast_list[j],S))
  psi_raw[[j]] <- select(SARI_combined_Survivor_link_x[[j]],starts_with('psi'))%>%as.matrix()%>%array(dim=c(n_sim,D,S))
  p_raw[[j]]<-array(dim=c(n_sim,Nowcast_list[j],D,S))
  delta_raw[[j]]<-select(SARI_combined_Survivor_link_x[[j]],starts_with('delta'))%>%as.matrix()%>%array(dim=c(n_sim,S))
  y_raw[[j]]<-select(SARI_combined_Survivor_link_x[[j]],starts_with('y'))%>%as.matrix()%>%array(dim=c(n_sim,S,Nowcast_list[j]))
  for(t in 1:(Nowcast_list[j])){
    for(s in 1:S){
      for(d in 1:D){
        p_raw[[j]][,t,d,s]<-eta_raw[[j]][,t,s]+psi_raw[[j]][,d,s] + delta_raw[[j]][,s]*log((y_raw[[j]][,s,t]+1)/population$population[s])
      }
    }
  }
...
y_plot
delta_plot
ggsave(delta_plot,file='predictions_delta_sari.pdf',width=9,height=6)
zeta_plot
xi_plot
psi_plot
eta_plot
...
ggplot(data=filter(check_residuals,s%in%federal_units[c(16,19,21,22,23)]),aes(x=t,y=(residual), colour=delay))+
...
ggplot(filter(check_residuals,s%in%federal_units[c(16,19,21,22,23)]),aes(x=log(total),y=(residual), colour=delay))+
  geom_point(alpha=0.2)+facet_wrap(~s,scales="free")+geom_smooth(alpha=0.2)+
  labs(x="Total counts (log)",y="Cumulative proportions residuals (probit)",title="SARI: cumulative proportions")


ggplot(filter(check_residuals,s%in%federal_units[c(5,6,7,8,9)]),aes(x=t,y=(residual), colour=delay))+
  geom_point(alpha=0.2)+facet_wrap(~s,scales="free")+geom_smooth(alpha=0.2)+
  labs(x="Time",y="Cumulative proportions residuals (probit)",title="SARI: cumulative proportions")

ggplot(filter(check_residuals,s%in%federal_units[c(5,6,7,8,9)]),aes(x=log(total),y=(residual), colour=delay))+
  geom_point(alpha=0.2)+facet_wrap(~s,scales="free")+geom_smooth(alpha=0.2)+
  labs(x="Total counts (log)",y="Cumulative proportions residuals (probit)",title="SARI: cumulative proportions")
...
load("~/media/alba/Disk 1/OneDrive/PhD/comp 2023 paper code/FInal_code/OMEGA CODE/SARI totals link/survivor_sea27.RData")
SARI_sea<-list()
SARI_combined_sea<-SARI_sea_quantiles<-list()
for(j in length(Nowcast_list):1){
  ...
  SARI_sea_quantiles[[j]]<-select(SARI_combined_sea[[j]],starts_with('y'))%>%as.matrix()%>%array(dim=c(n_sim,S,Nowcast_list[j]))%>%
    apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','s','t'),value.name='y')%>%#mutate(t=t+Nowcast_list[j]-W)%>%
    spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_units[s])
...
load("~/media/alba/Disk 1/OneDrive/PhD/comp 2023 paper code/FInal_code/OMEGA CODE/SARI totals link/survivor_delta_spline27.RData")
SARI_Survivor_spline_x<-list()
SARI_spline_delta<-list()
for(j in length(Nowcast_list):1){
  # Survivor age model
  SARI_Survivor_spline_x[[j]]<-as.mcmc.list(SARI_spline_output_delta[[j]])
  SARI_spline_delta[[j]] <- as_tibble(do.call('rbind',SARI_Survivor_spline_x[[j]]))
  SARI_spline_quantiles[[j]]<-select(SARI_spline_delta[[j]],starts_with('y'))%>%as.matrix()%>%array(dim=c(n_sim,S,Nowcast_list[j]))%>%
    apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','s','t'),value.name='y')%>%#mutate(t=t+Nowcast_list[j]-W)%>%
    spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_units[s])
}
...
for(j in 1:length(Nowcast_list)){
  SARI_nowcasts_compare[[j]] <- rbind(
    full_join(SARI_logspline_quantiles[[j]],filter(y_all, t%in%c((1):Nowcast_list[j])),by=c('s','t'))%>%mutate(m=c('SARI delta link'),d=t-Nowcast_list[j],now=j),
    full_join(SARI_spline_quantiles[[j]],filter(y_all, t%in%c((1):Nowcast_list[j])),by=c('s','t'))%>%mutate(m=c('SARI delta spline'),d=t-Nowcast_list[j],now=j),
    full_join(SARI_sea_quantiles[[j]],filter(y_all, t%in%c((1):Nowcast_list[j])),by=c('s','t'))%>%mutate(m=c('SARI no delta'),d=t-Nowcast_list[j],now=j)
  )%>%filter(d>-D)
}
...
SARI_nowcast_dmax<-SARI_nowcast_dmax%>%mutate(t=as.numeric(t),`2.5%`=as.numeric(`2.5%`),`50%`=as.numeric(`50%`),`97.5%`=as.numeric(`97.5%`),
                                                  m=factor(m,levels=c('SARI delta link','SARI delta spline','SARI no delta'
                                                  )),y=as.numeric(y),
...
SARI_nowcast_dmax_summary<-SARI_nowcast_dmax%>%group_by(m,d,s)%>%
  ...
  mutate(m=factor(m,levels=c('SARI delta link','SARI delta spline','SARI no delta'
  )),
...
totals_SARI <- ggplot(SARI_nowcast_dmax_summary_average)+
