...
for(i in 1:n_chains){
  ...
  sari_inits[[i]] <- list(kappa_alpha=rnorm(sari_constants$K_t,0,0.1),
...
SARI_time<-system.time({
  ...
  sari_samples <- as.mcmc.list(foreach(i=1:n_chains)%dopar%{
    runMCMC(sari_compiled_mcmc[[i]],niter=1200000,nburnin=200000,inits=sari_inits[[i]],nchains=1,samplesAsCodaMCMC = TRUE,thin=1000)
  })
...
ggsave(forecast_plot,file='Plots/sari_forecast_nona.pdf',width=9,height=2.5)
...
save(sari_samples,file='sari_samples_nona.RData')
