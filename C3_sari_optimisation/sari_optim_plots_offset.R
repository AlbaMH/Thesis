...
#ggsave(forecast_plot,file='Plots/sari_forecast1.pdf',width=9,height=2.5)
forecast_plot
...
run_sari_nona_cluster<- function( seed, sari_constants, sari_data, sari_inits ) {
  ...
  list(theta_opt=theta_opt_gamma2, phi_opt=phi_opt_gamma2)
}
...
region_all <- ggplot(filter(sari_y,s%in%c(2,17,15)))+
  ...
  labs(x=NULL,y=NULL,title='Predicted Weekly Total SARI Cases',
...
#ggsave(region_all,filename = 'Covid plots/SARI_5000_l.png',width=12,height=3)
...
ggsave(theta_opt,filename = 'SARI_theta_opt_GDM_5000.png',width=8,height=4)
theta_opt
...
phi_opt<-ggplot(data=filter(phi_df))+ 
  ...
  theme(strip.text = element_text(size=12),
        plot.caption = element_text(size=10),plot.title=element_text(size=16),
        axis.text = element_text(size=10),axis.title = element_text(size=12))
ggsave(phi_opt,filename = 'SARI_phi_opt_5000.png',width=21,height=4)
phi_opt
