...
n_chains <- 8 # Number of MCMC chains to run. Reduce if you don't have more than 8 CPU cores!
...
ggsave(bar_plot,file='Sari opt plots/sari_bar_plot.pdf',width=4.5,height=3)
...
sari_constants <- list(N=N,C=C,S=S,K_t=n_knots[1]-1,K_w=n_knots[2]-2,w=week_index[1:N])
...
for(i in 1:n_chains){
  ...
  for(s in 1:S){
    for(t in 1:C){
      for(d in 1:D){
        if(is.na(sari_data$z[t,s,d])) sari_inits[[i]]$z[t,s,d]=rpois(1,median(sari_data$z[,s,d],na.rm=T))
      }
...
SARI_time<-system.time({
  ...
  sari_samples <- as.mcmc.list(foreach(i=1:n_chains)%dopar%{
    runMCMC(sari_compiled_mcmc[[i]],niter=2000000,nburnin=1000000,inits=sari_inits[[i]],nchains=1,samplesAsCodaMCMC = TRUE,thin=1000)
  })
  
})

# save samples
save(sari_samples,file='sari_samples.RData')
...
sari_iota <- select(sari_combined_samples,starts_with('iota'))%>%as.matrix()%>%matrix(nrow=n_sim,ncol=S)%>%
  apply(2,quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','s'),value.name='y')%>%
  spread(quantile,y)
...
forecast_plot
#ggsave(forecast_plot,file='Sari opt plots/sari_forecast_og4.pdf',width=9,height=2.5)

# Predictions for the three most populous regions.
forecast_plot_other1 <- ggplot(filter(sari_y,s%in%c(5,12)))+
  geom_ribbon(aes(x=dates[t],ymin=`2.5%`,ymax=`97.5%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`10%`,ymax=`90%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`17.5%`,ymax=`82.5%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`25%`,ymax=`75%`,fill=as.factor(s)),alpha=0.25)+
  geom_line(aes(x=dates[t],y=`50%`,colour=as.factor(s)))+
  geom_point(data=filter(y_data,s%in%c(5,12)),aes(x=dates[t],y=y,colour=as.factor(s)))+
  facet_wrap(~r,nrow=1,scales='free')+
  labs(x=NULL,y=NULL,title='Predicted Weekly Total SARI Cases')+guides(colour=FALSE,fill=FALSE)+
  #coord_cartesian(xlim=c(dates[200],dates[230]))+
  geom_vline(xintercept=dates[C])+theme_minimal()+
  scale_fill_viridis_d(begin=0.2,end=0.8)+scale_colour_viridis_d(begin=0.2,end=0.8)+
  theme(strip.text = element_text(size=12))+scale_x_date(labels = date_format("%b"),date_breaks = '2 months')
#ggsave(forecast_plot,file='Sari opt plots/sari_GDM_forecast_other11.pdf',width=14,height=2.5)
forecast_plot_other1

forecast_plot_other2 <- ggplot(filter(sari_y,s%in%c(13,16)))+
  geom_ribbon(aes(x=dates[t],ymin=`2.5%`,ymax=`97.5%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`10%`,ymax=`90%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`17.5%`,ymax=`82.5%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`25%`,ymax=`75%`,fill=as.factor(s)),alpha=0.25)+
  geom_line(aes(x=dates[t],y=`50%`,colour=as.factor(s)))+
  geom_point(data=filter(y_data,s%in%c(13,16)),aes(x=dates[t],y=y,colour=as.factor(s)))+
  facet_wrap(~r,nrow=1,scales='free')+
  labs(x=NULL,y=NULL,title='Predicted Weekly Total SARI Cases')+guides(colour=FALSE,fill=FALSE)+
  #coord_cartesian(xlim=c(dates[200],dates[230]))+
  geom_vline(xintercept=dates[C])+theme_minimal()+
  ...
  theme(strip.text = element_text(size=12))+scale_x_date(labels = date_format("%b"),date_breaks = '2 months')
#ggsave(forecast_plot,file='Sari opt plots/sari_GDM_forecast_other11.pdf',width=14,height=2.5)
forecast_plot_other2

forecast_plot_other2 <- ggplot(filter(sari_y,s%in%c(6,22)))+
  geom_ribbon(aes(x=dates[t],ymin=`2.5%`,ymax=`97.5%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`10%`,ymax=`90%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`17.5%`,ymax=`82.5%`,fill=as.factor(s)),alpha=0.25)+
  geom_ribbon(aes(x=dates[t],ymin=`25%`,ymax=`75%`,fill=as.factor(s)),alpha=0.25)+
  geom_line(aes(x=dates[t],y=`50%`,colour=as.factor(s)))+
  geom_point(data=filter(y_data,s%in%c(6,22)),aes(x=dates[t],y=y,colour=as.factor(s)))+
  facet_wrap(~r,nrow=1,scales='free')+
  labs(x=NULL,y=NULL,title='Predicted Weekly Total SARI Cases')+guides(colour=FALSE,fill=FALSE)+
  #coord_cartesian(xlim=c(dates[200],dates[230]))+
  geom_vline(xintercept=dates[C])+theme_minimal()+
...
# Run the independent time series model for each region.
seed <- c(486583,385876,282842,173274)
independent_models <- list()
#for(s in 1:S){
 # independent_models[[s]]=run_gdm_survivor(D,N,C,n_knots=c(18,9),censored_sari[,s,],niter=20000,nburnin=10000,thin=10,
 #                                          nchains=2,seed=seed)
#}

# Extract replicates of the total cases from the independent models.
independent_y <- do.call('cbind',lapply(independent_models,function(x)x$replicates$y[,1:(C-D_max+1)]))%>%
  array(dim=c(dim(independent_models[[1]]$replicates$y)[1],C-D_max+1,S))
# Replicate totals for the whole state from the independent models.
independent_Y <- apply(independent_y,c(1,2),sum)

# Replicate sample means and variances for the whole state.
Y_statistics <- rbind(tibble(x=apply(independent_Y,1,mean),model='Independent',statistic='Sample Mean'),
                      tibble(x=apply(independent_Y,1,var),model='Independent',statistic='Sample Variance'),
                      tibble(x=apply(Y_replicates,1,mean),model='Joint',statistic='Sample Mean'),
                      tibble(x=apply(Y_replicates,1,var),model='Joint',statistic='Sample Variance'))

# Plot the posterior replicate sample means and variances from the joint model
# verus the independent models.
replicate_plot <- ggplot(Y_statistics)+stat_density(aes(x=x,fill=model),position = 'identity',trim=TRUE,adjust=1.5,alpha=0.5)+facet_wrap(~statistic,scales='free')+
  geom_vline(data=tibble(x=c(var(Y_data$y[1:(C-D_max+1)]),mean(Y_data$y[1:(C-D_max+1)])),statistic=c('Sample Variance','Sample Mean')),aes(xintercept=x))+
  labs(x='',y='Density',title='Replicates of Total Reported SARI Cases',subtitle='Across All 22 Health Regions')+
  scale_fill_viridis_d(name='Model',begin=0.2,end=0.8)+theme_minimal()+theme(strip.text = element_text(size=12),legend.text = element_text(size=10),legend.position=c(0.775,1.35))+
  guides(fill=guide_legend(nrow=2,title.position = 'left',keyheight = unit(0.65,'lines'),keywidth=unit(0.65,'lines')))
#ggsave(replicate_plot,filename = 'Sari opt plots/sari_replicate_plot.pdf',width=4.5,height=2.25)
replicate_plot
...
# lambda
plot(sari_samples[,'lambda[224, 12]'], main="lambda[224,12]")
plot(sari_samples[,'lambda[230, 12]'], main="lambda[230,12]")
plot(sari_samples[,'lambda[229, 12]'], main="lambda[229,12]")
plot(sari_samples[,'theta[12]'], main="theta[12]")
plot(sari_samples[,'delta[229, 12]'], main="delta[229, 12]")
