...
n_chains <- 8 # Number cores to run. Reduce if you don't have more than 8 CPU cores!
...
f<-function(i, censored_sari,N,C,S,D, population){
  ...
  # Standardized log population offset for each region 
  pop_offset<-(log(population)-mean(log(population)))/sd(log(population))
  # Constants (e.g. number of weeks to model, number of knots) for NIMBLE.
  sari_constants_NBz <- list(N=N,C=C,S=S,K_t=n_knots[1]-1,K_w=n_knots[2]-2, D=D_rem,obs_index=obs_index,w=week_index[1:N], log_pop=pop_offset[i])
  ...
  sari_gamma_code <- nimbleCode({
    for(s in 1:S){
      for(t in 1:N){
        # Mean total cases.
        log(lambda[t,s]) <- log_pop + iota[s] + delta[t,s] + xi[w[t],s]
        for(d in 1:(D-1)){
          # Expected cumulative proportions.
          probit(p[t,s,d]) <- beta[s,d]+gamma[t,s]
        }
        p[t,s,D]<-1
        # Expected proportions reported (Negative-Binomial means).
        mu[t,s,1]<- p[t,s,1]
        for(d in 2:D){
 ...
 # iota_opt_offset<-iota_opt-log_pop
  ...
  for(t in 1:N){
    for(s in 1:S){
      ...
      lambda_opt[t,s]<-exp(pop_offset[i]+iota_opt[s]+delta_opt[t,s]+xi_opt[week_index[t],s])
      for(d in 1:(D_rem-1)){
...
stat_t=1
for(i in 1:S_all){
  # png(paste('Sari opt plots/R',i,'sari_no_gam.png'), width = 700, height = 350)
...
# Constants (e.g. number of weeks to model, number of knots) for NIMBLE.
sari_constants <- list(N=N,C=C,S=S,K_t=n_knots[1]-1,K_w=n_knots[2]-2, D=D,obs_index=obs_index,w=week_index[1:N],log_pop=(log(population)-mean(log(population)))/sd(log(population)))
# Data (e.g. total cases, partial counts, spline model matrix) for NIMBLE.
sari_data <- list(z=censored_sari[1:C,,1:D],y=apply(censored_sari,c(1,2),sum)[1:C,],X_t=blank_jagam$jags.data$X[,2:(n_knots[1])],S_t=blank_jagam$jags.data$S1,
                  X_w=blank_jagam$jags.data$X[,(n_knots[1]+1):(n_knots[1]+n_knots[2]-2)],S_w=blank_jagam$jags.data$S2,
                  zeros=rep(0,max(sari_constants$K_t,sari_constants$K_w)))
...
run_sari_nona_cluster<- function( seed,MCMC_seed, sari_constants, sari_data, sari_inits ) {
  ...
  sari_code <- nimbleCode({
    for(s in 1:S){
      for(t in 1:N){
        # Mean total cases.
        log(lambda[t,s]) <-  log_pop[s] + iota[s] + delta[t,s] + xi[w[t],s]
        for(d in 1:D){
          # Expected cumulative proportions.
          probit(p[t,s,d]) <- beta[s,d]+gamma[t,s]
        }
  ...
  #set random seed for MCMC chain
  set.seed(i)
  n_chains<-1
  #MCMC_seed<-ceiling(runif(n_chains,1,1000))  ##change seed to random number generated seed- determined outside cluster?
  sari_samples <- runMCMC(sari_compiled_mcmc,niter=380000,nburnin=30000,inits=sari_inits,nchains=n_chains,samplesAsCodaMCMC = TRUE,thin=500, setSeed = MCMC_seed[seed])
  return(sari_samples)
}
...
time_nona_cluster<-system.time({
  sari_samples <- parLapply(cl = sari_nona_cluster, X = 1:n_chains, 
                            MCMC_seed=ceiling(runif(n_chains,1,1000)),
...
#times 
time_optim 
time_nona_cluster+time_optim 
(time_nona_cluster+time_optim)[3]/60/60
...
ggsave(forecast_plot,file='Plots/sari_nona_optim_forecast.pdf',width=9,height=2.5)
forecast_plot
...
plot(sari_samples[,'delta[180, 10]'], main="optim delta[180, 10]")
plot(sari_samples[,'delta[77, 18]'], main="optim delta[77, 18]")
#gamma
...
plot(sari_samples[,'xi[43, 21]'], main="optim xi[43, 21]")
plot(sari_samples[,'xi[36, 13]'], main="optim xi[36, 13]")
#theta
