...
# 
# # Create data frame for 2021 and 2022 SARI cases. 
...
# 
# # Maximum number of weeks in the data.
...
# # Maximum number of delays in data 
# D_N<-dim(SARI_delay)[2]-2
# 
# # Order weeks and regions in data.
...
# 
# # Set D_max - the maximum delay we shall consider. 
# D_max<-20
# # Set D - number of delays we shall explicitly model. 
# D<-8
# 
# # Array for partial SARI counts by week, region and delay 
# SARI_z<-SARI_array[,,1:(D_max)]
# # Matrix for total SARI counts by region and week.
...
W=60 
# Select nowcast dates for rolling experiment. 
...
R<-30 
# Select number of chains for MCMC. 
n_chains=2
# Select number of iterations for MCMC chains.
n_iter=15000
...
SARI_surv_region <- function(seed, SARI_x, SARI_z, Nowcast_list, W, S, R, D, D_max, n_knots, n_iter, n_burn, n_thin, n_chains){
  # Load libraries.
  library(nimble)
  library(tidyverse)
  library(mgcv)
...
# Run cluster. 
time_surv_regions<-system.time({
  SARI_surv <- parLapply(cl = this_cluster_surv, X = 1:length(Nowcast_list), 
                                fun = SARI_surv_region,
...
# Save Samples.
save(SARI_surv,time_surv_regions,file='cluster_survivor_x.RData')
#
...
for(k in 1:length(sari_parameter_group)){
  ...
  for(j in 1:length(Nowcast_list)){
    sari_parameter_names <- colnames((as.matrix(SARI_surv[[j]])))
    sari_index <- which(startsWith(sari_parameter_names,sari_parameter_group[k]))
    sari_psrf[[k]][[j]]<- sapply(sari_index,function(v)gelman.diag(SARI_surv[[j]][,v],autoburnin = FALSE,transform = TRUE,multivariate = FALSE)$psrf[,1])
