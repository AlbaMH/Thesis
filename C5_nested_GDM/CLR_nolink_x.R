...
# 
# # Calculate total SARI cases across all delays.
...
# 
# 
# # set D_max - the maximum delay we consider.
# D_max<-20
...
SARI_CLR_region <- function(seed, SARI_x, SARI_z, Nowcast_list, W, S, R, D, D_max, n_knots, n_iter, n_burn, n_thin, n_chains){
  ...
  SARI_code <- nimbleCode({
    for(s in 1:S){
      ...
      for(t in 1:N){
        ...
        logit(mu[t,s]) <- beta0[s] + beta1[t,s]  
  ...
  SARI_inits <- list(
    ...
    kappa_gamma=matrix(rnorm(S*(SARI_constants$K_td),0,0.01),ncol=S),
  ...
  for(s in 1:S){
    SARI_mcmc_config$addSampler(target=c(paste('kappa_zeta[1:',(SARI_constants$K_t2),', ',s,']', sep=''),paste('sigma_zeta[',s,']',sep=''),paste('zeta0[',s,']',sep=''),
                                         ...
                                         paste('omega[',s,']',sep='')),type='AF_slice') 
...
# CLR model - no link between SARI and COVID-positive SARI porportion.
...
# Run the cluster.
time_nolink<-system.time({
  SARI_clr_nolink <- parLapply(cl = this_cluster_region, X = 1:length(Nowcast_list), 
                                fun = SARI_CLR_region,
...
# Save Samples.
save(SARI_clr_nolink,time_nolink,file='cluster_clr_nolink_x.RData')
...
for(k in 1:length(sari_parameter_group)){
  ...
  for(j in 1:length(Nowcast_list)){
    sari_parameter_names <- colnames((as.matrix(SARI_clr_nolink[[j]])))
    sari_index <- which(startsWith(sari_parameter_names,sari_parameter_group[k]))
    sari_psrf[[k]][[j]]<- sapply(sari_index,function(v)gelman.diag(SARI_clr_nolink[[j]][,v],autoburnin = FALSE,transform = TRUE,multivariate = FALSE)$psrf[,1])
