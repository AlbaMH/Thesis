...
#                        trim_ws = TRUE)
# 
# # Select attributes of interest from data. 
...
# 
# # Make data frame for 2021 SARI cases.
...
# 
# # Make data frame for 2022 SARI cases.
...
# 
# # Make data frame for 2021 and 2022 SARI cases.
...
# 
# # Create attribute for number of weeks reporting was delayed by (delay weeks).
...
# 
# # Create matrix with onset week as rows and delay weeks as columns:
# # Sum individual cases over week, delay and region.
...
# 
# # Maximum number of weeks in data.
...
# # Maximum delay observed in data. 
# D_N<-dim(SARI_delay)[2]-2
# 
# # Order SARI cases data frame by region and week.
...
# 
# # Save spatial regions - federal units. 
...
# 
# 
# # set D_max - maximum number of delays we consider. 
# D_max<-20
# # set D - number of delays to explicitly model.
# D<-8
# 
# # Array for partial SARI counts by week, region and delay.
# SARI_z<-SARI_array[,,1:(D_max)]
# # Matrix for total SARI counts by week and region.
...
W=60  
# Select nowcast dates for rolling prediction experiment.
...
n_iter=15000
# Set amount of MCMC chain burn-in.
n_burn=10000
# Select amount of MCMC chain thinning.
n_thin=5
...
# Simulate observed x values for each nowcasting date.
censored_x<-list()
...
SARI_CLR_region <- function(seed, SARI_x, SARI_z, Nowcast_list, W, S, R, D, D_max, n_knots, n_iter, n_burn, n_thin, n_chains){
    ...
    for(s in 1:S){
      ...
      for(d in 2:D){
        for(t in 1:obs_index[s,d]){
          # Model for delayed SARI counts (after second delay).
      ...
      for(t in 1:N){
        ...
        # Linear predictor for COVID-positve Beta-Binomial probability.
  ...
  # Set up spline bases.
  # Tensor product spline for time-delay interaction for absolute SARI proportions reported. 
  ...
  # Cubic splne with shrinkage for mean SARI cases.
  ...
  # Cubic spline with shrinkage for proportion of COIVD-posive SARI cases. 
  ...
  SARI_mcmc_config <- configureMCMC(SARI_model,monitors=c("mu","lambda","zeta0","beta0","y","x",
                                                          "gamma0","chi","theta","phi","upsilon",
                                                          "sigma_gamma","sigma_zeta", "sigma_beta",
                                                          'pi','beta1',"zeta1","omega","delta",
                                                          "kappa_zeta","kappa_beta","kappa_gamma"),
                                    useConjugacy = FALSE)
...
# Run the cluster. 
time_clr_regions<-system.time({
  SARI_clr_output2 <- parLapply(cl = this_cluster_region, X = 1:length(Nowcast_list), 
                            fun = SARI_CLR_region,
...
# Save Samples.
save(SARI_clr_output2,time_clr_regions,file='cluster_clr_x.RData')
#
...
for(k in 1:length(sari_parameter_group)){
  ...
  for(j in 1:length(Nowcast_list)){
    sari_parameter_names <- colnames((as.matrix(SARI_clr_output2[[j]])))
    sari_index <- which(startsWith(sari_parameter_names,sari_parameter_group[k]))
    sari_psrf[[k]][[j]]<- sapply(sari_index,function(v)gelman.diag(SARI_clr_output2[[j]][,v],autoburnin = FALSE,transform = TRUE,multivariate = FALSE)$psrf[,1])
