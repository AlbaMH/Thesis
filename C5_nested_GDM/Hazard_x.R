...
# 
# # Make a data frame for 2021 and 2022.
...
# # Any missing entires mean no (zero) cases were reported.
...
# # Number of weeks we shall model.
# N<-max(SARI_delay$SEM_PRI)
# # Maximum number of delay weeks observed in data.
# D_N<-dim(SARI_delay)[2]-2
# 
# # Order data frame by weeks and regions. 
...
# 
# # set D_max - maximum delay we will consider. 
# D_max<-20
# # set D - number of delays we will explicitly model.
# D<-8
# 
# # Array for partial SARI cases by region, week and delay.
# SARI_z<-SARI_array[,,1:(D_max)]
# # Matrix for total SARI cases by region and week.
...
# 
# # Earliest date of reported case in data:
...
# # Matrix for COVID cases by region and week.
...
R<-30  
# Select number of MCMC chains.
n_chains=2
# Select iterations for MCMC chains.
n_iter=15000
# Select amount of burn in for MCMC chains.
n_burn=10000
# Select amount of thinning for MCMC chains.
n_thin=5
  ...
  SARI_code <- nimbleCode({
    for(s in 1:S){
      ...
      for(t in 1:N){
        ...
        logit(nu[t,1,s]) <- psi[1,s] + eta[t,1,s] 
        for(d in 2:D){
          logit(nu[t,d,s]) <-   psi[d,s] + eta[t,d,s] 
        }
        # Linear predictor for expected mean total SARI cases. 
...
# Run cluster. 
time_hazard<-system.time({
  SARI_clr_haz <- parLapply(cl = this_cluster_region, X = 1:length(Nowcast_list), 
                                fun = SARI_CLR_region,
...
# Save Samples.
save(SARI_clr_haz,time_hazard,file='cluster_hazard_x.RData')
#
...
for(k in 1:length(sari_parameter_group)){
  ...
  for(j in 1:length(Nowcast_list)){
    sari_parameter_names <- colnames((as.matrix(SARI_clr_haz[[j]])))
    sari_index <- which(startsWith(sari_parameter_names,sari_parameter_group[k]))
    sari_psrf[[k]][[j]]<- sapply(sari_index,function(v)gelman.diag(SARI_clr_haz[[j]][,v],autoburnin = FALSE,transform = TRUE,multivariate = FALSE)$psrf[,1])
