...
Dengue_t<-tibble(t=1:N)%>%mutate(time_bin=(floor(t/100)+1))
...
Dengue_surv_bin_region <- function(seed, Dengue_z, Dengue_t, population_order, Nowcast_list, S,  D, D_max, n_knots, n_iter, n_burn, n_thin, n_chains){
  # Load libraries.
  library(nimble)
  library(tidyverse)
  library(mgcv)
      ...
      for(t in 1:N){
       ...
       # index_delta[t,s]<-delta_converter(x=(log((y[s,t]+1)/pop[s])),grid =delta_grid[1:length_delta], grid_length = length_delta)[1]
        for(d in 1:D){
          ...
          probit(p[t,d,s]) <- psi[d,s] + eta[t,s] + delta[s,time_bin[t]]*log((y[s,t]+1)/pop[s])
        }
  ...
  Dengue_constants <- list(N=N,
                           S=S,
                           D=D,
                           time_bin=as.numeric(unlist(Dengue_t[1:N,2])),
                           B=length(unique(as.numeric(unlist(Dengue_t[1:N,2])))),
  ...
  Dengue_inits <- list(
    ...
    delta=matrix(rnorm(Dengue_constants$B*S,0,0.1),nrow=S),#matrix(rnorm(D*S,0,1),nrow=D),
...
#    sigma_delta=runif(2,0,1),#matrix(runif(2*S,0,1),nrow=2),
  ...
  for(s in 1:S){
    ...
    Dengue_mcmc_config$addSampler(target=c(paste('delta[',s,', 1:',Dengue_constants$B,']', sep=''),
...
this_cluster_bin<- makeCluster(n_cores)



# Note: takes approximately 
time_bin<-system.time({
  Dengue_surv_bin <- parLapply(cl = this_cluster_bin, X = 1:length(Nowcast_list), 
                                      fun = Dengue_surv_bin_region,
...
stopCluster(this_cluster_bin)

#SAVE Samples
save(Dengue_surv_bin,time_bin,file='Dengue_surv_bin_D4.RData')
#


plot(Dengue_surv_bin[[2]][,"zeta[3, 1]"],main="zeta[3, 1]")
plot(Dengue_surv_bin[[2]][,"int_zeta[1]"],main="int_zeta[1]")
plot(Dengue_surv_bin[[2]][,"zeta[100, 1]"],main="zeta[100, 1]")
plot(Dengue_surv_bin[[2]][,"xi[10, 1]"],main="xi[10, 1]")
plot(Dengue_surv_bin[[2]][,"xi[1, 1]"],main="xi[1, 1]")
plot(Dengue_surv_bin[[2]][,"delta[5]"],main="delta[5]")
plot(Dengue_surv_bin[[2]][,"delta[3]"],main="delta[3]")
plot(Dengue_surv_bin[[1]][,"delta[1]"],main="delta[1]")
plot(Dengue_surv_bin[[1]][,"delta[2]"],main="delta[2]") 
plot(Dengue_surv_bin[[1]][,"delta[5]"],main="delta[5]")
plot(Dengue_surv_bin[[1]][,"delta[6]"],main="delta[6]")
plot(Dengue_surv_bin[[1]][,"psi[1, 1]"],main="psi[1, 1]")
plot(Dengue_surv_bin[[1]][,"eta[3, 1]"],main="eta[1, 1]")

plot(Dengue_surv_bin[[2]][,"psi[2, 1]"],main="psi[2, 1]")
plot(Dengue_surv_bin[[2]][,"psi[3, 1]"],main="psi[3, 1]")
plot(Dengue_surv_bin[[2]][,"psi[4, 1]"],main="psi[4, 1]")
plot(Dengue_surv_bin[[2]][,"psi[4, 1]"],main="psi[4, 1]")
plot(Dengue_surv_bin[[2]][,"psi[2, 2]"],main="psi[2, 2]")


#min(select(as_tibble(do.call('rbind',as.mcmc.list(Dengue_output_cluster[[1]]))),starts_with('zeta')))
...
for(k in 1:length(Dengue_parameter_group)){
  ...
  for(j in 1:length(Nowcast_list)){
    Dengue_parameter_names <- colnames((as.matrix(Dengue_surv_bin[[j]])))
    Dengue_index <- which(startsWith(Dengue_parameter_names,Dengue_parameter_group[k]))
    Dengue_psrf[[k]][[j]]<- sapply(Dengue_index,function(v)gelman.diag(Dengue_surv_bin[[j]][,v],autoburnin = FALSE,transform = TRUE,multivariate = FALSE)$psrf[,1])
...
for(j in length(Nowcast_list):1){
  Dengue_logy[[j]]<-as.mcmc.list(Dengue_surv_bin[[j]])
...
for(j in length(Nowcast_list):1){
  ...
  lambda[[j]] <- select(Dengue_combined_logy[[j]],starts_with('lambda'))%>%as.matrix()%>%array(dim=c(n_sim,Nowcast_list[j],S))%>%
  ...
  Dengue_bin_quantiles[[j]]<-select(Dengue_combined_logy[[j]],starts_with('y'))%>%as.matrix()%>%array(dim=c(n_sim,S,Nowcast_list[j]))%>%
    apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','s','t'),value.name='y')%>%#mutate(t=t+Nowcast_list[j]-W)%>%
    spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_guide$abbrev_state[s])
  
  y_plot<-y_plot+
    geom_line(data=Dengue_bin_quantiles[[j]], aes(x=t, y=`50%`, colour=Nowcast_date))+
    geom_ribbon(data=Dengue_bin_quantiles[[j]], aes(x=t, ymin=`2.5%`, ymax=`97.5%`,fill=Nowcast_date))
  
  
  # Plot Zeta Variables 
  zeta[[j]] <- select(Dengue_combined_logy[[j]],starts_with('zeta'))%>%as.matrix()%>%array(dim=c(n_sim,Nowcast_list[j],S))%>%
    apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','t','s'),value.name='y')%>%#mutate(t=t+Nowcast_list[j]-W)%>%
    spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_guide$abbrev_state[s])
  ...
  xi[[j]] <- select(Dengue_combined_logy[[j]],starts_with('xi'))%>%as.matrix()%>%array(dim=c(n_sim,52,S))%>%
  ...
  delta[[j]] <- select(Dengue_combined_logy[[j]],starts_with('delta'))%>%as.matrix()%>%array(dim=c(n_sim,S))%>%
  ...
  #omega[[j]] <- select(Dengue_combined_logy[[j]],starts_with('omega'))%>%as.matrix()%>%array(dim=c(n_sim,W,S))%>%
  #  apply(c(2,3),quantile,c(0.025,0.5,0.975))%>%melt(varnames=c('quantile','t','s'),value.name='y')%>%mutate(t=t+Nowcast_list[j]-W)%>%
  # spread(quantile,y)%>%mutate(Nowcast_date=paste(Nowcast_list[j]),s=federal_guide$abbrev_state[s])
  ...
  psi[[j]] <- select(Dengue_combined_logy[[j]],starts_with('psi'))%>%as.matrix()%>%array(dim=c(n_sim,D,S))%>%
...
check_ae_plot<-ggplot()+
  ...
  theme_minimal()+labs(y="Dengue Hospitalisations", x="Time", title="CLR Regional Model")+#+geom_vline(xintercept = 51)
  scale_colour_discrete(name='Nowcast Date')+scale_fill_discrete(name='Nowcast Date')+
  geom_line(data=filter(Dengue_bin_quantiles[[4]],t%in%c(371:376)), aes(x=t, y=`50%`, colour=s))+
  facet_wrap(~s)+scale_x_continuous(limits=c(371,376))+
  geom_ribbon(data=filter(Dengue_bin_quantiles[[4]],t%in%c(371:376)), aes(x=t, ymin=`2.5%`, ymax=`97.5%`,fill=s), alpha=0.2)

check_ae<-full_join(filter(Dengue_bin_quantiles[[4]],t%in%c(373)),filter(y_all,t%in%c(373)))%>%mutate(ae=`50%`-y)
...
for(j in length(Nowcast_list):1){
  Dengue_sea[[j]]<-as.mcmc.list(Dengue_surv_logysmooth[[j]])
  # Combine all MCMC chains.
...
for(j in 1:length(Nowcast_list)){
  dengue_nowcasts_compare[[j]] <- rbind(
    full_join(Dengue_bin_quantiles[[j]],filter(y_all, t%in%c((1):Nowcast_list[j])),by=c('s','t'))%>%mutate(m=c('Dengue delta link'),d=t-Nowcast_list[j],now=j),
    full_join(Dengue_sea_quantiles[[j]],filter(y_all, t%in%c((1):Nowcast_list[j])),by=c('s','t'))%>%mutate(m=c('Dengue spline delta'),d=t-Nowcast_list[j],now=j)
  )%>%filter(d>-D)
}
...
dengue_nowcast_dmax<-dengue_nowcast_dmax%>%mutate(t=as.numeric(t),`2.5%`=as.numeric(`2.5%`),`50%`=as.numeric(`50%`),`97.5%`=as.numeric(`97.5%`),
                                                  m=factor(m,levels=c('Dengue delta link'#,'Dengue spline delta'
                                                  )),y=as.numeric(y),
...
dengue_nowcast_dmax_summary<-dengue_nowcast_dmax%>%group_by(m,d,s)%>%
  ...
  mutate(m=factor(m,levels=c('Dengue delta link'#,'Dengue spline delta'
  )),
