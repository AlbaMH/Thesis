---
title: "COVID No NA optim model versions"
...
```{r GDM cluster theta optim region}
...
disp_df<-data.frame( exp(covid_inits_gam$log_theta))
colnames(disp_df)<-c("theta_opt_gamma_og")
...
phi_df<-data.frame(melt(exp(covid_inits_gam$log_phi)))
colnames(phi_df)<-c("r","d","phi_opt_gamma_og")
...
covid_gamma_code <- nimbleCode({
  for(s in 1:S){
    ...
    for(s in 1:S){
      for(d in 1:D){
        for(t in 1:obs_index[s,d]){
         prob[t,s,d]<-(theta[s])/(theta[s]+p_gamma[t,s]*mu[t,s,d])
         z[t,s,d] ~ dnegbin( prob[t,s,d], theta[s])  
        }
       }
      }
    for(s in 1:S){
...
gamma_inits<-apply(z_inits,2,rowSums)
  ...
  covid_compiled_model_gamma <- compileNimble(covid_model_gamma)
  # Set up the MCMC.
  covid_mcmc_config_gamma<- configureMCMC(covid_model_gamma, monitors=c('beta','xi','gamma','delta','iota', 'lambda', 'theta','p_gamma'),useConjugacy = FALSE)
...
S_all<-7
D_max<-14
time_optimcluster2_theta<-system.time({
optim_output  <- parLapply(cl = this_cluster_optim, X = 1:S_all, fun = run_optim_allcode, 
                           N=N, S=1, D=6, C=C, n_knots=n_knots, 
                           partial_counts=z[1:N,1:S_all,1:D_max])
})
...
time_optimcluster2_theta
time_optimcluster2_theta[3]/60
#Plot optimisation 
stat_t=1
for(i in 1:S_all){
  plot(stat_t:N, y_full[stat_t:N,i])
...
## no gamma term optim model


```{r GDM no gam cluster optim region}
...
covid_gamma_code <- nimbleCode({
  for(s in 1:S){
    ...
    for(s in 1:S){
      for(d in 1:D){
        for(t in 1:obs_index[s,d]){
         prob[t,s,d]<-(theta[s])/(theta[s]+lambda[t,s]*mu[t,s,d])
         z[t,s,d] ~ dnegbin( prob[t,s,d], theta[s])  
        }
       }
      }
...
for(t in 1:N){
  ...
  covid_compiled_model_gamma <- compileNimble(covid_model_gamma)
  # Set up the MCMC.
  covid_mcmc_config_gamma<- configureMCMC(covid_model_gamma, monitors=c('beta','xi','gamma','delta','iota', 'lambda', 'theta'),useConjugacy = FALSE)
...
S_all<-7
D_max<-14
time_optimcluster2_no_gam<-system.time({
optim_output  <- parLapply(cl = this_cluster_optim, X = 1:S_all, fun = run_optim_allcode, 
                           N=N, S=1, D=6, C=C, n_knots=n_knots, 
                           partial_counts=z[1:N,1:S_all,1:D_max])
})
...
time_optimcluster2_no_gam
time_optimcluster2_no_gam[3]/60
#Plot optimisation 
stat_t=1
for(i in 1:S_all){
  plot(stat_t:N, y_full[stat_t:N,i])
...
theme(text = element_text(size=14), axis.title = element_text(size=16), plot.title = element_text(size=20),plot.subtitle=element_text(size=18),
  ...
  scale_x_date(date_labels = '%d %b',breaks=dates[c(5,19,33)])
region_all
#ggsave(region_all,filename = 'Covid plots/region_all_5000.pdf',width=10,height=6)
